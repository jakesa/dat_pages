...
module DATPages
  ...
  module WebDriver::PageObjects::ElementContainer
    ...
    def element(*args)
      ...
      class_eval(%Q(private def #{args[:name].to_s}; @#{args[:name].to_s} ||= DATPages::WebDriver::PageObjects::Element.new("#{args[:find_by].nil? ? args[:find_by] : args[:find_by].to_sym}}");end))
    end
